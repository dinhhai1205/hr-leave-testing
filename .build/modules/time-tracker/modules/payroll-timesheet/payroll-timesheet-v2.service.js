...
exports.PayrollTimeSheetServiceV2 = void 0;
...
let PayrollTimeSheetServiceV2 = class PayrollTimeSheetServiceV2 extends database_1.TypeOrmBaseService {
    constructor(payrollTimeSheetRepository, timeSheetAdjustmentService, workScheduleService, employeeService, prtrxHdrService, prtrxEmpService, payrollGroupService, workScheduleAssignmentService) {
    ...
    async createMultiPayrolls(createPayrollDto, companyId, userEmail) {
        ...
        const createDtos = workScheduleEntities.map(workScheduleEntity => {
            const totalScheduledWorkHours = workScheduleEntity.totalScheduledWorkHours;
            const totalScheduledWorkDays = workScheduleEntity.totalScheduledWorkDays;
            return {
    ...
    async createPayrollsForEmployeesInPrtrxHdr(createQuery, prtrxHdrId, companyId, userEmail, updateRegularWorkDays) {
        ...
        return this.getPayrollsByHeaderId(prtrxHdrId, companyId, createQuery?.payrollCalculationMethod || database_1.PayCalculationMethod.Daily, createQuery, updateRegularWorkDays ? updateRegularWorkDays : false);
    }
    ...
    async updatePayrollsWorksheduledDays(prtrxHdrId, companyId, userEmail) {
        ...
        const createDtos = await Promise.all(workScheduleEntities.map(async (workScheduleEntity) => {
            ...
            let totalPayrollRegularWorkDays = workScheduleEntity.payCalcMet === 1
                ? totalScheduledWorkDays
                : totalScheduledWorkHours;
            if (prId) {
    ...
    async getPayrollsByHeaderId(payrollHeaderId, companyId, payrollCalculationMethod = database_1.PayCalculationMethod.Daily, paginationQueryDto, updateRegularWorkDays = true) {
        const payrollAlias = database_1.ETableName.PAYROLL_TIME_SHEET;
        const employeeAlias = 'employee';
        const organizationStructureAlias = database_1.ETableName.ORGANIZATION_STRUCTURE;
        const payrollGroupAlias = database_1.ETableName.PAYROLL_GROUP;
    ...
    async getAllPayrollsByHeaderId(payrollHeaderId, companyId, paginationQueryDto, updateRegularWorkDays = true) {
        const payrollAlias = database_1.ETableName.PAYROLL_TIME_SHEET;
        const employeeAlias = 'employee';
        const organizationStructureAlias = database_1.ETableName.ORGANIZATION_STRUCTURE;
        const payrollGroupAlias = database_1.ETableName.PAYROLL_GROUP;
        ...
        const allAdjustments = await this.timeSheetAdjustmentService.getAdjustmentsByPayrollHeaderIdWithoutPagination(payrollHeaderId, companyId);
        const results = payrolls.map(value => {
            const adjustments = allAdjustments.filter(adjustment => adjustment.payrollTimesheetId === value.id);
            const unpaid = [];
            const adjustmentDaysAddition = [];
            const adjustmentDaysDeduction = [];
            for (const adjustment of adjustments) {
    ...
    async getDayScheduleOfDay(day, workScheduleId, companyId) {
        ...
        const workScheduleEntity = await this.workScheduleService.getWorkScheduleByWorkScheduleId(workScheduleId, companyId);
        if (!workScheduleEntity?.daySchedules)
            return null;
        return workScheduleEntity.daySchedules.find(daySchedule => daySchedule.day === weekDay);
    }
};
exports.PayrollTimeSheetServiceV2 = PayrollTimeSheetServiceV2;
exports.PayrollTimeSheetServiceV2 = PayrollTimeSheetServiceV2 = __decorate([
...
], PayrollTimeSheetServiceV2);
//# sourceMappingURL=payroll-timesheet-v2.service.js.map